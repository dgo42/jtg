...
public class ColorerActionContributor extends TextEditorActionContributor {
	 ...
	 */
	Action						cursorRegionAction;

	Action				hrcupdateAction;
			...
			for (int idx = 0; idx < ftypes.length; idx++) {
				...
				if (cftype.equals(ftypes[idx])) {
					...
					if (mgroup.getParentItem().getParent().getParentItem() != null) {
						mgroup.getParentItem().getParent().getParentItem()
								.setImage(ImageStore.EDITOR_CUR_GROUP.createImage());
					}
	...
	void showPairError() {
		MessageDialog.openInformation(null, Messages.get("editor.pairerr.title"), Messages.get("editor.pairerr.msg"));
	}
