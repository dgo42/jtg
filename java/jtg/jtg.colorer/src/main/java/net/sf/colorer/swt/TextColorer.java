...
public class TextColorer {
	...
	public boolean selectPair() {
		...
		if (cp.end == null) return false;
		if (cp.topPosition)
			text.setSelection(text.getOffsetAtLine(cp.sline) + cp.start.start, text.getOffsetAtLine(cp.eline) + cp.end.end);
		else
			text.setSelection(text.getOffsetAtLine(cp.eline) + cp.end.start, text.getOffsetAtLine(cp.sline) + cp.start.end);
		return true;
	}
	...
	public boolean selectContentPair() {
		...
		if (cp.end == null) return false;
		if (cp.topPosition)
			text.setSelection(text.getOffsetAtLine(cp.sline) + cp.start.end, text.getOffsetAtLine(cp.eline) + cp.end.start);
		else
			text.setSelection(text.getOffsetAtLine(cp.eline) + cp.end.end, text.getOffsetAtLine(cp.sline) + cp.start.start);
		return true;
	}
	...
	void pairsDraw(GC gc, PairMatch pm) {
		...
		if (pm.start != null) {
			...
			pairDraw(gc, (StyledRegion) pm.start.rdef, pm.start.start + lineOffset, pm.start.end + lineOffset);
		}
		if (pm.end != null) {
	...
	class InternalHandler implements VerifyListener, ControlListener, TextChangeListener, SelectionListener, MouseListener,
			KeyListener, DisposeListener, LineStyleListener, LineBackgroundListener, PaintListener, TraverseListener {
		...
		public void lineGetStyle(LineStyleEvent e) {
			...
			for (int idx = 0; idx < lrarr.length; idx++) {
				...
				StyleRange sr = new StyleRange(start, end, cm.getColor(rdef.bfore, rdef.fore), cm.getColor(rdef.bback,
		...
		public void lineGetBackground(LineBackgroundEvent e) {
			...
			for (int idx = 0; idx < lr.length; idx++) {
				...
				if (lr[idx].end == -1 && rdef != null) e.lineBackground = cm.getColor(rdef.bback, rdef.back);
			}
		}
