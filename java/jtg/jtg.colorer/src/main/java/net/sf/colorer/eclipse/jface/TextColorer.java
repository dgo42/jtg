...
public class TextColorer {
	 ...
	 */
	class AsyncReconcyler implements IPresentationReconciler, ITextListener, ITextInputListener, Runnable {
		...
		IRegion getDamageRegion(DocumentEvent event) {
			...
			try {
				if (Logger.TRACE) {
					Logger.trace("CDR",
							"getDamageRegion: sending modify event: " + fDocument.getLineOfOffset(event.getOffset()));
				}
		...
		IRegion getVisualDamageRegion(TextEvent event) {
			if (Logger.TRACE) {
				Logger.trace("CDR", "getVisualDamageRegion TextEvent: " + event.getOffset() + ":" + event.getLength()
						+ ", replace:" + event.getReplacedText());
			}
			try {
			...
			try {
				...
				for (int lno = l_start; lno <= l_end; lno++) {
					...
					for (int idx = 0; idx < lrarr.length; idx++) {
						...
						//                        /*
						Logger.trace("CDR", "trace line:" + lno + "lineofset=" + lineinfo.getOffset() + "linelength="
								+ lineinfo.getLength() + " start=" + start + " length=" + length);
						...
						StyleRange sr = new StyleRange(start, length, getDeepFGColor(fDeepLevelCounter, lno, rdef),
								getDeepBGColor(fDeepLevelCounter, lno, rdef), rdef.style);
		...
		public void run() {
			while (fViewer != null && !fDisplay.isDisposed()) {
				fDisplay.asyncExec(new Runnable() {
					public void run() {
						if (System.currentTimeMillis() > fModTimestamp + ASYNC_DELAY - 200) repairPresentation(false);
					}
				});
		...
		public void repairPresentation(boolean visual) {
			...
			try {
				...
				if (Logger.TRACE) {
					Logger.trace("CDR",
							"Presentation build for region: " + visibleDamage.getOffset() + ":" + visibleDamage.getLength());
				}
				...
				int widgetLength = fProjectionViewer.modelOffset2WidgetOffset(visibleDamage.getOffset()
	 ...
	 * BaseEditor API
	 */
	class WidgetEventHandler implements KeyListener, DisposeListener, LineBackgroundListener, PaintListener,
			TraverseListener, ControlListener, MouseListener, ISelectionChangedListener, IViewportListener {
		...
		public void lineGetBackground(LineBackgroundEvent e) {
			...
			if (horzCrossColor != null && horzCross && lineHighlighting && e.lineOffset <= caret
					&& caret <= e.lineOffset + length) {
				e.lineBackground = fColorManager.getColor(((StyledRegion) horzCrossColor).bback,
	...
	public void setCross(boolean horz, boolean vert) {
		...
		vertCrossColor = null;
		horzCrossColor = null;
		if (horzCross) horzCrossColor = fBaseEditor.getHorzCross();
		if (vertCross) vertCrossColor = fBaseEditor.getVertCross();
		fViewer.invalidateTextPresentation();
	}
	...
	public boolean matchPair() {
		...
		try {
			...
			if (currentPair.end == null) return false;
	...
	public boolean selectPair() {
		...
		try {
			...
			if (currentPair.topPosition) {
				...
				fEditor.selectAndReveal(offset, fDocument.getLineOffset(currentPair.eline) + currentPair.end.end - offset);
			} else {
				int offset = fDocument.getLineOffset(currentPair.eline) + currentPair.end.start;
				fEditor.selectAndReveal(offset, fDocument.getLineOffset(currentPair.sline) + currentPair.start.end - offset);
			}
			return true;
		} catch (BadLocationException e) {
			Assert.isTrue(false, "Never reach");
	...
	public boolean selectContentPair() {
		...
		try {
			...
			if (currentPair.topPosition) {
				...
				fEditor.selectAndReveal(offset, fDocument.getLineOffset(currentPair.eline) + currentPair.end.start - offset);
			} else {
				int offset = fDocument.getLineOffset(currentPair.eline) + currentPair.end.end;
				fEditor.selectAndReveal(offset, fDocument.getLineOffset(currentPair.sline) + currentPair.start.start
						- offset);
			}
			return true;
		} catch (BadLocationException e) {
	...
	void stateChanged() {
		...
		try {
			...
			if (!pairsHighlighting) {
				...
				PairMatch newmatch = fBaseEditor.getPairMatch(curLine, curOffset - lineOffset);
				if (newmatch != null) fBaseEditor.searchLocalPair(newmatch);
				if ((newmatch == null && currentPair != null) || (newmatch != null && !newmatch.equals(currentPair))) {
					pairsDraw(null, currentPair);
		...
		if (gc != null) {
			...
			if (sr != null) {
				if (highlightStyle == HLS_XOR) {
					int resultColor = sr.fore ^ fColorManager.getColor(text.getBackground());
					if (text.getLineAtOffset(text.getCaretOffset()) == text.getLineAtOffset(start) && horzCross
							&& horzCrossColor != null && ((StyledRegion) horzCrossColor).bback)
	...
	void pairsDraw(GC gc, PairMatch pm) {
		...
		try {
			if (pm.start != null) {
				...
				pairDraw(gc, (StyledRegion) pm.start.rdef, pm.start.start + drawStart, pm.start.end + drawStart);
			}
			if (pm.end != null) {
				...
				int drawStart = fProjectionViewer.modelOffset2WidgetOffset(lineOffset);
				if (drawStart == -1) return;
				pairDraw(gc, (StyledRegion) pm.end.rdef, pm.end.start + drawStart, pm.end.end + drawStart);
			}
		} catch (BadLocationException e) {
			Assert.isTrue(false, "Never reach");
		}
	...
	void drawLine(int lno) {
		if (lno < 0 || lno >= fDocument.getNumberOfLines()) return;
