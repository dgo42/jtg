...
public class ParseTreeOutliner implements IWorkbenchOutlineSource, RegionHandler, EditorListener, IWorkbenchAdapter {
	...
	Vector<OutlineElement>			regionsList		= new Vector<OutlineElement>();
	Vector<OutlineElement>			cList			= regionsList;

	Stack<Vector<OutlineElement>>	schemeStack		= new Stack<Vector<OutlineElement>>();
	...
	public void clear(Vector<OutlineElement> list, int topLine) {
		int csize = 0;
		while (csize < list.size() && list.elementAt(csize).lno < topLine) {
			csize++;
		}
		list.setSize(csize);
	}
	...
	public void enterScheme(int lno, String line, int sx, int ex, Region region, String scheme) {
		...
		Vector<OutlineElement> elements = null;
		...
		if (cList.size() > 0 && cList.lastElement() instanceof OutlineSchemeElement) {
			...
			if (sel != null) {
				elements = (Vector<OutlineElement>) sel.elements;
				clear(elements, lno);
			}
		}
		if (sel == null || elements == null) {
			elements = new Vector<OutlineElement>();
