...
public class ColorerEditor extends TextEditor implements IPropertyChangeListener, IColorerEditorAdapter {
	...
	VerifyListener						tabReplacer;

	IColorerReloadListener				fReloadListener;
	...
	public ColorerEditor() {
		tabReplacer = new VerifyListener() {
		...
		fReloadListener = new IColorerReloadListener() {
		...
		IAction action = new ResourceAction(Messages.getResourceBundle(), "Editor.RulerPreferencesAction.") {
			public void run() {
				...
				if (preferencePages.length > 0 && (shell == null || !shell.isDisposed())) {
					PreferencesUtil.createPreferenceDialogOn(shell, preferencePages[0], preferencePages,
							fTextColorer.getFileType()).open();
				}
			}
		};
		setAction(ITextEditorActionConstants.RULER_PREFERENCES, action);

		action = new TextOperationAction(Messages.getResourceBundle(), "Projection.Toggle.", this, ProjectionViewer.TOGGLE,
				true);
		action.setActionDefinitionId(IFoldingCommandIds.FOLDING_TOGGLE);
		setAction(action.getActionDefinitionId(), action);

		action = new TextOperationAction(Messages.getResourceBundle(), "Projection.ExpandAll.", this,
				ProjectionViewer.EXPAND_ALL, true);
		action.setActionDefinitionId(IFoldingCommandIds.FOLDING_EXPAND_ALL);
		setAction(action.getActionDefinitionId(), action);

		action = new TextOperationAction(Messages.getResourceBundle(), "Projection.CollapseAll.", this,
	...
	protected void initializeKeyBindingScopes() {
		setKeyBindingScopes(new String[] {"net.sf.colorer.eclipse.ColorerScope"});
	}
	...
	protected void editorContextMenuAboutToShow(IMenuManager parentMenu) {
		...
		if (!fTextColorer.pairAvailable()) return;
	...
	public void propertyChange(PropertyChangeEvent e) {
		...
		if (e == null || e.getProperty().equals(PreferencePage.PAIRS_MATCH)) {
			...
			if (pairs.equals("PAIRS_NO")) pmode = TextColorer.HLS_NONE;
	...
	public BaseEditor getBaseEditor() {
		if (fBaseEditor == null) {
			fBaseEditor = new CachedBaseEditor(ColorerPlugin.getDefaultPF(), new DocumentLineSource(getSourceViewer()
					.getDocument()));
