...
public class FileTypePreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
		...
		public void modify(Object element, String property, Object value) {
			...
			if (element.equals(ColorerPlugin.HRD_SIGNATURE)) {
				...
				if (i == 0) {
					...
					ColorerPlugin.getDefault().setPropertyHRD(currentType, hrdList.elementAt(i - 1));
				}
			}
			if (element.equals(ColorerPlugin.WORD_WRAP_SIGNATURE)) {
				int i = ((Integer) value).intValue();
	...
	Vector<String>		hrdList					= (Vector<String>) ColorerPlugin.getDefault().getHRDList();
	...
	public Control createContents(Composite parent) {
		...
		{
			typePropertiesViewer = new TableViewer(composite, SWT.H_SCROLL | SWT.V_SCROLL | SWT.BORDER | SWT.FULL_SELECTION);
			...
			for (int idx = 1; idx < valuesHRD.length; idx++) {
				valuesHRD[idx] = pf.getHRDescription("rgb", hrdList.elementAt(idx - 1));
			}
		}
		return composite;
	}
