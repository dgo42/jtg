...
public class FoldingBuilder {
	...
	class InternalRegionHandler implements RegionHandler, EditorListener {
		...
		Stack<FoldingElement>	schemeStack	= new Stack<FoldingElement>();
		...
		public void leaveScheme(int lno, String line, int sx, int ex, Region region, String scheme) {
			...
			if (!schemeStack.empty() && last.s_line == ((FoldingElement) schemeStack.peek()).s_line) return;
	...
	public void install(BaseEditor baseEditor, IFoldingReciever reciever) {
		...
		fReciever = reciever;
		fBaseEditor.addRegionHandler(fHandler, fBaseEditor.getParserFactory().getHRCParser().getRegion("def:EmbeddedTag"));
