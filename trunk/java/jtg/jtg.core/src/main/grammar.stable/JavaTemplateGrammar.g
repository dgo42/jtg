...
directive_include
    ...
    : DIRECTIVE_BEGIN v:DIRECTIVE_CMD_INCLUDE
      file=directive_attr_file (params=directive_attr_params)? (arg=directive_attr_arg)?
      MACROCODE_END (WS)? 
      {
		unit.addInclude(v, file, arg, params); 
	  }
    ;
...
directive_attr_params returns[String params]
	{
		params = "";
	}
	: DIRECTIVE_ATTR_PARAMS params=directive_attr
	;
...
script
    : SCRIPT_BEGIN MACROCODE_END (WS)? ( v:TARGETCODE 
	    { 
		  unit.addScript(v); 
	    }
