...
public class HRCParser {
	...
	public synchronized Group[] getGroups() {
		...
		for (Enumeration<FileType> e = enumerateFileTypes(); e.hasMoreElements();) {
			FileType ft = e.nextElement();
	 ...
	 */
	public Enumeration<FileType> enumerateFileTypes() {
		return new Enumeration<FileType>() {
			int	idx	= 0;
