...
public class GeneratorDialog {
	 ...
	 */
	public void setFileList(final Vector<String> list) {
		fileList.removeAll();
		for (int idx = 0; idx < list.size(); idx++) {
			fileList.add(list.elementAt(idx));
		}
	 ...
	 * internal name.
	 */
	public void setHRDSchema(Vector<String> list) {
		for (int idx = 0; idx < list.size(); idx += 2) {
			hrdSchema.add(list.elementAt(idx));
			hrdSchema.setData(list.elementAt(idx), list.elementAt(idx + 1));
		}
		hrdSchema.select(0);
	}
