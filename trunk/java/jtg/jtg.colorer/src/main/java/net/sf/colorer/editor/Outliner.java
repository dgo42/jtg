...
public class Outliner implements IOutlineSource {
	protected Vector<OutlineItem>	fOutlineStorage	= new Vector<OutlineItem>();
	...
	public Outliner(Region searchRegion) {
		...
		fEditorListener = new EditorListener() {
			public void modifyEvent(int topLine) {
				int new_size;
				for (new_size = fOutlineStorage.size() - 1; new_size >= 0; new_size--) {
					if (((OutlineItem) fOutlineStorage.elementAt(new_size)).lno < topLine) break;
				}
	...
	static int manageTree(Vector<Integer> treeStack, int newLevel) {
		while (treeStack.size() > 0 && newLevel < (treeStack.lastElement()).intValue()) {
			treeStack.removeElementAt(treeStack.size() - 1);
		}
		if (treeStack.size() == 0 || newLevel > (treeStack.lastElement()).intValue()) {
	...
	// -----------------------------------------
	Vector<OutlineListener>	listeners	= new Vector<OutlineListener>();
