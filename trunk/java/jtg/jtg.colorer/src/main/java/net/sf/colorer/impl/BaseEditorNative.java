...
public class BaseEditorNative implements BaseEditor {
	...
	Vector<EditorListener>	editorListeners	= new Vector<EditorListener>();
	...
	protected void finalize() throws Throwable {
		if (!disposed) {
			if (Logger.TRACE) {
				Logger.trace("BaseEditor",
						"Colorer:BaseEditorNative: Invalid global state: dispose() should be called explicitly!");
			}
			// Memory leak instead???
			// Is this safe???
			dispose();
	...
	public PairMatch getPairMatch(int lineNo, int linePos) {
		...
		for (int idx = 0; idx < lrArray.length; idx++) {
			...
			if ((l1.region.hasParent(defPairStart) || l1.region.hasParent(defPairEnd)) && linePos >= l1.start
					&& linePos <= l1.end) pair = l1;
		};
		if (pair != null) {
