...
public class ColorerFoldingProvider {
	...
	class FoldingReciever implements IFoldingReciever {
		...
		public void notifyInvalidate(int lineno) {
			...
			while (iter.hasNext()) {
				...
				deletions.addElement((ProjectionAnnotation) ann);
			}
			fLastLine = lineno;
		}
		...
		public void notifyFoldingItem(int s_line, int s_offset, int e_line, int e_offset, String schema) {
			...
			try {
				...
				for (Enumeration<ProjectionAnnotation> e = deletions.elements(); e.hasMoreElements();) {
					ProjectionAnnotation deleted = e.nextElement();
	...
	private Vector<ProjectionAnnotation>			deletions				= new Vector<ProjectionAnnotation>();
	private HashMap<ProjectionAnnotation, Position>	additions				= new HashMap<ProjectionAnnotation, Position>();
