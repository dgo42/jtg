...
public class ColorerContentOutlinePage extends ContentOutlinePage implements OutlineListener {
	...
	Action				fLinkToEditorAction;

	ISelectionListener	thisSelectionListener;
	...
	private Vector<ISelectionChangedListener>	selectionListeners	= new Vector<ISelectionChangedListener>();
	...
	public ColorerContentOutlinePage() {
		fLinkToEditorAction = new Action("Link", Action.AS_CHECK_BOX) {
			public void run() {
				prefStore.setValue("outline.link", isChecked());
			}
		};

		thisSelectionListener = new ISelectionListener() {
			...
			public void selectionChanged(IWorkbenchPart part, ISelection selection) {
				...
				if (!fLinkToEditorAction.isChecked() || sel == null || !(sel instanceof ITextSelection)) return;
	...
	public void attach(ColorerEditor editor) {
		...
		fActiveOutlineSource.setHierarchy(ColorerPlugin.getDefault().getPreferenceStore().getBoolean("Outline.Hierarchy"));
